...
> **2026-07-01 更新**: v1.0.7 リリース完了。画像挿入機能（BMP/PNG、トレース用下絵/恒久貼付）・A3用紙サイズ対応・図面テンプレート保存・オートセーブ機能・ショートカットキー設定機能・マウスレス要素配置（キーボード配置モード）を追加、それに伴うキーボード入力不具合（Enter/矢印/数字キー・Escape・Space→PageUp/Down・ショートカットキー設定のキー捕捉）を一連修正（commits e6d1a66..2066bb7）。
> **2026-07-01 更新**: v1.0.6 リリース完了。R2R 起動クラッシュ2件・テストモード背景色バグを修正（commits f9681d9/3283b65/83582ef）。
...
**既知の制限事項（2026-07-01・保留・殿承認済み）**: 枠ラベルをダブルクリックで編集開始した直後に別操作（クリック等）を行うと、WinUI3側の既知バグ（[microsoft-ui-xaml Issue #6179](https://github.com/microsoft/microsoft-ui-xaml/issues/6179)、Microsoft側not planned）によりフォーカスが暗黙的に奪われ、まれに空ラベルのまま確定される場合がある。実用上の影響は限定的なため保留。詳細調査は [docs-notes/keyboard-input-fix-plan.md](../docs-notes/keyboard-input-fix-plan.md) ラウンド6〜8参照。
...
## アプリ全般
- [x] **パッケージング・配布** — Inno Setup インストーラー（`installer/GuiEcad_Setup.iss`）で対応済み。詳細は [docs/packaging.md](packaging.md) §5。MSIX 化は保留のまま（§6 将来検討）。
...
| 不具合修正（2026-07-01, commits f9681d9/3283b65/83582ef） | R2R 起動クラッシュ修正2件: ToggleButton 複合 XAML コンテンツ(0x802b000a)・OnToolSelected の Canvas.Invalidate 直接呼び出し（DispatcherQueue ラムダ経由に変更）／テストモード OFF 時にツールバー背景色が戻らない不具合(ClearValue で ThemeResource バインディング復活) |
