...
- **ファイル拡張子: `.GCAD`**（確定）。
- **保存対象**: `DocumentInfo` / `Settings` / `DeviceTable`（BOM=型式/メーカー/数量を含む） / `Library` / `Sheets`（Elements・Connectors・Frames・FreeLines・Lines・GridSpec・BusConfig・MainCircuit）。
  - `FreeLines`（主回路の自由直線・mm座標）・`MainCircuit`（主回路モード）は 2026-06-21 追加。旧ファイルは空配列/false で互換。
